#!/usr/bin/env python3
"""Verschluesselt eine HTML-Seite zu einer passwortgeschuetzten Seite (PBKDF2 600k + AES-256-GCM).
Uebernommen aus dem Trend-Radar-Projekt, nur Beschriftung und Salt-Datei geaendert.
Aufruf: python encrypt_page.py input.html output.html "passwort"
"""
...
ITERATIONS = 600_000
SALT_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "seite-salt.bin")
  ...
  try {{
    ...
    if (remember) {{
      try {{
        ...
        localStorage.setItem("tr_key", btoa(String.fromCharCode(...new Uint8Array(raw))));
      }} catch (e) {{}}
    }}
    show(html);
  }} catch (e) {{
